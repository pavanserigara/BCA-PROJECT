...
// Fetch linked student's teachers to show as default contacts
...
$stmt_contacts = $pdo->prepare("
    ...
    JOIN students std ON s.course_id = std.course_id AND s.semester = std.semester
    LEFT JOIN (
        SELECT other_id,
...
");
$stmt_contacts->execute([$me, $me, $me, $me, $student_id]);
...
<div class="mb-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
    <div>
        <h2 class="text-3xl font-black text-slate-800 dark:text-white tracking-tight leading-none italic uppercase">Communication Relay</h2>
        <p class="text-slate-500 dark:text-slate-400 mt-4 font-medium italic">Direct interface with institutional faculty and coordinators.</p>
    </div>
</div>
...
<div class="grid grid-cols-1 lg:grid-cols-12 gap-10 h-[700px] mb-20 pb-10">
    ...
    <div class="lg:col-span-4 bg-white dark:bg-slate-800 rounded-[3.5rem] shadow-premium border border-slate-100 dark:border-slate-800 flex flex-col overflow-hidden">
        <div class="p-8 border-b border-slate-50 dark:border-slate-700/50 bg-slate-50/50 dark:bg-slate-900/50">
            <h4 class="text-xs font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest italic">Faculty Directory</h4>
        </div>
        
        <div class="flex-1 overflow-y-auto p-4 space-y-2 custom-scroll">
            <?php if (empty($contacts)): ?>
                <div class="p-10 text-center text-slate-400 italic font-bold">No linked faculty members found.</div>
            <?php else: ?>
                <?php foreach ($contacts as $c): ?>
                    <?php $active = (int)$c['id'] === $selected_user_id; ?>
                    <a href="messaging.php?user=<?php echo $c['id']; ?>" class="flex items-center gap-4 p-5 rounded-[2.5rem] transition-all <?php echo $active ? 'bg-emerald-600 text-white shadow-premium' : 'hover:bg-slate-50 dark:hover:bg-slate-900 border border-transparent'; ?>">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-[10px] <?php echo $active ? 'text-white/70' : 'text-slate-400'; ?> font-bold uppercase tracking-widest truncate"><?php echo $c['last_message'] ?: 'Initiate Protocol'; ?></p>
                        </div>
                    </a>
                <?php endforeach; ?>
            <?php endif; ?>
        ...
        <?php if ($selected_user): ?>
            ...
            <div class="p-8 border-b border-slate-900 bg-slate-950/50 backdrop-blur-xl flex items-center justify-between z-10 sticky top-0">
                <div class="flex items-center gap-4">
                    <div class="w-12 h-12 rounded-2xl bg-emerald-600 flex items-center justify-center text-white font-black text-lg italic shadow-xl">
                    ...
                    <div>
                        ...
                        <span class="text-[9px] font-black text-emerald-500 uppercase tracking-[0.2em] italic"><?php echo $selected_user['role']; ?> Personnel</span>
                    </div>
                </div>
            </div>
            ...
            <div class="flex-1 overflow-y-auto p-10 space-y-8 custom-scroll" id="chat-stream">
                <?php if (empty($messages)): ?>
                    <div class="h-full flex flex-col items-center justify-center text-slate-600">
                        <i class="fas fa-satellite text-5xl mb-6 opacity-20"></i>
                        <p class="text-sm font-black uppercase tracking-widest italic">Synchronize First Message</p>
                    </div>
                <?php else: ?>
                    <?php foreach ($messages as $m): ?>
                        ...
                        <div class="flex <?php echo $is_me ? 'justify-end' : 'justify-start'; ?>">
                            <div class="max-w-[85%] md:max-w-[70%]">
                                <div class="p-6 rounded-[2.5rem] <?php echo $is_me ? 'bg-emerald-600 text-white rounded-tr-none shadow-premium' : 'bg-slate-900 text-slate-300 rounded-tl-none border border-slate-800'; ?>">
            ...
            <div class="p-8 border-t border-slate-900">
                <form method="POST" class="flex gap-4">
                    ...
                    <input type="text" name="message" required autocomplete="off" placeholder="Type transmission..."
                        class="flex-1 px-8 py-5 bg-slate-900 border border-slate-800 rounded-[2rem] text-sm font-bold text-white italic outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500/20 transition-all">
                    <button type="submit" class="w-16 h-16 bg-emerald-600 text-white rounded-[1.5rem] flex items-center justify-center shadow-xl hover:bg-emerald-700 transition-all transform active:scale-90">
        ...
        <?php else: ?>
            <div class="h-full flex flex-col items-center justify-center text-slate-600 p-10 text-center">
                <div class="w-24 h-24 bg-slate-900 rounded-[2.5rem] flex items-center justify-center mb-8 border border-slate-800">
                    <i class="fas fa-user-slash text-4xl opacity-10"></i>
                </div>
                <h4 class="text-xl font-black text-white italic uppercase tracking-tight mb-3">Relay Destination Undefined</h4>
                <p class="text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] max-w-xs">Select a verified faculty member from the directory to establish a direct communication link.</p>
            </div>
        <?php endif; ?>
    </div>
</div>
